...
VALID_USER_INPUT = {
    ...
    CONF_FROM_ENTITIES: ["calendar.parent1", "calendar.parent2"],
...
async def test_user_flow_rejects_to_calendar_as_its_own_source(hass):
    bad_input = {
        **VALID_USER_INPUT,
        CONF_FROM_ENTITIES: ["calendar.kids", "calendar.parent1"],
    }
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
...
@pytest.mark.asyncio
async def test_user_flow_requires_keywords_or_copy_all_from(hass):
    ...
    assert result2["type"] == FlowResultType.FORM
    assert result2["errors"]["base"] == "no_keywords_or_copy_all_from"


@pytest.mark.asyncio
async def test_user_flow_rejects_copy_all_from_outside_from_entities(hass):
    bad_input = {
        **VALID_USER_INPUT,
        CONF_FROM_ENTITIES: ["calendar.parent1"],
        CONF_COPY_ALL_FROM: ["calendar.parent2"],  # not in from_entities
    }
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
    ...
    assert result2["type"] == FlowResultType.FORM
    assert result2["errors"][CONF_COPY_ALL_FROM] == "copy_all_from_not_in_from_entities"
